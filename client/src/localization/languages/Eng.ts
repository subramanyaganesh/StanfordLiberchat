...
export default {
  ...
  com_nav_welcome_message: `Welcome to the Stanford Al Playground, a platform for safely interacting with large language models (LLMs).
You are likely familiar with generative artificial intelligence, or Gen Al, which is built using algorithms that can generate text, images, and custom code in response to prompts. LLMs are a type of generative Al that specializes in text-based content.
The Stanford AI Playground is a service for the Stanford community that provides a secure environment supporting a wide range of Al providers, including OpenAl, Anthropic, and Google.
Please note: The data you provide are for your use only, and remain private when interacting with the service. Any text or files you upload will not be saved to the vendor, or alter the LLM in any way. Don't use high-risk data. Remember, while LLMs are advanced tools, they are not flawless and may create errors or hallucinations. Take caution before using results verbatim.
Begin by typing your question or prompt in the box below. You can customize settings or switch between various LLMs from the options at the top of this page.`,
